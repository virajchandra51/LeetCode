...
public:
    ...
    Node* BuildBST(int pre[], int* indx, int n, int min, int max)
    {
        if((*indx) >= n)
          return NULL;
        ...
        if(pre[*indx] > min and pre[*indx] < max)
        {
            root = newNode(pre[*indx]);
            *indx = *indx + 1;
            // cout<<"h "<<*indx<<endl;
            if(*indx < n)
              root->left = BuildBST(pre, indx, n, min, root->data);
            //  cout<<"z "<<*indx<<endl;
            if(*indx < n)
              root->right = BuildBST(pre, indx, n, root->data, max);
        }
    ...
    Node* post_order(int pre[], int size)
    {
        ...
        return BuildBST(pre, &indx, size, INT_MIN, INT_MAX);
